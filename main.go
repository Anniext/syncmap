...
func (g *Generator) Mutate() (err error) {
	...
	// 遍历注释列表，删除目标注释
	for i, commentGroup := range f.Comments {
		for _, comment := range commentGroup.List {
			if strings.TrimSpace(comment.Text) == "//go:build !goexperiment.synchashtriemap" {
				// 删除注释
...
func (g *Generator) Types() map[string]func(*ast.TypeSpec) {
	return map[string]func(*ast.TypeSpec){
		"Map": func(t *ast.TypeSpec) {
			l1 := t.Type.(*ast.StructType).Fields.List[1]
			l1.Type = expr("sync.Mutex", l1.Type.Pos())
			g.replaceKey(t.Type)
			l2 := t.Type.(*ast.StructType).Fields.List[0]
			l2.Type = expr("sync.Mutex", l2.Type.Pos())
			g.replaceKey(t.Type)
		},
